...
class TestTerraformAWSComponents:
    ...
    def test_aws_multiple_components(self):
        ...
        assert_otm_component(otm, 3, 'docker-container', 'service_task', self.public_cloud_id,
                             ['aws_ecs_task_definition'])
        assert_otm_component(otm, 4, 'load-balancer', 'lb', None, ['aws_lb'])
        assert_otm_component(otm, 5, 'load-balancer', 'wu-tang', None, ['aws_elb'])
        assert_otm_component(otm, 6, 'aws-lambda-function', 'test_lambda', self.public_cloud_id,
                             ['aws_lambda_function'])
        ...
        assert_otm_component(otm, 11, 'sqs-simple-queue-service', 'terraform_queue', self.public_cloud_id,
                             ['aws_sqs_queue'])
        ...
        assert_otm_component(otm, 16, 'elastic-container-kubernetes', 'example', self.public_cloud_id,
                             ['aws_eks_cluster'])
        ...
        assert_otm_component(otm, 20, 'step-functions', 'my_sfn_state_machine', self.public_cloud_id,
    ...
    def test_aws_security_groups_components(self):
        ...
        iac_data = FileUtils.get_byte_data(filename).decode()
        mapping_filename = test_resource_paths.default_terraform_aws_mapping
        iac_to_otm = IacToOtm('Test case AWS security groups components', 'aws_security_groups_components',
                              IacType.TERRAFORM)
        iac_to_otm.run(IacType.TERRAFORM, [FileUtils.get_data(mapping_filename)], [iac_data])
        ...
        assert_otm_component(otm, 19, 'generic-client', '0.0.0.0/0', self.internet_id,
                             ['Outbound connection destination IP'])
        assert_otm_component(otm, 20, 'generic-client', '255.255.255.255/32', self.internet_id,
                             ['Outbound connection destination IP'])
    ...
    def test_aws_dataflows(self):
        ...
        assert_otm_component(otm, 1, 'aws-lambda-function', 'basic_lambda', self.public_cloud_id,
                             ['aws_lambda_function'])
        ...
        assert_otm_component(otm, 6, 'sqs-simple-queue-service', 'failure_queue', self.public_cloud_id,
                             ['aws_sqs_queue'])
