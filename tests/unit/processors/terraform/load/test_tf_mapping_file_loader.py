...
class TestTerraformMappingFileLoader(TestCase):

    @patch('yaml.load')
    def test_single_valid_file(self, yaml_load_mock):
        # GIVEN a single valid Terraform mapping file
        mapping_file_data = [bytes('VALID TF FILE', 'utf-8')]
        ...
        mappings = TerraformMappingFileLoader(mapping_file_data).load()
    ...
    def test_multiple_valid_files(self, yaml_load_mock):
        # GIVEN two valid Terraform mapping file
        mapping_file_data = [bytes('VALID TF FILE', 'utf-8'), bytes('VALID TF FILE', 'utf-8')]
    ...
    def test_invalid_file(self, yaml_load_mock):
        # GIVEN an invalid Terraform mapping file
        mapping_file_data = [bytes('INVALID TF FILE', 'utf-8')]
        ...
        with self.assertRaises(LoadingMappingFileError) as loading_error:
            TerraformMappingFileLoader(mapping_file_data).load()

        # AND The error info is right
        assert str(loading_error.exception.title) == 'Error loading the TF mapping file. The mapping files are not valid.'
    ...
    def test_some_empty_mapping_file(self, yaml_load_mock):
        ...
        mapping_file_data = [bytes('VALID TF FILE', 'utf-8'), bytes('', 'utf-8')]
