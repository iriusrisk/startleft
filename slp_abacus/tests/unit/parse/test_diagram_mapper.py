...
from slp_abacus.slp_abacus.parse import diagram_mapper
from slp_abacus.slp_abacus.parse.diagram_mapper import DiagramMapper, _find_mapping
from slp_abacus.tests.util.builders import build_diagram, build_component, build_components
...
class TestDiagramMapper:
    ...
    def test_default_trustzone_creation(self, trustzone_mappings: List[dict], expected_tz_index: int):
        # GIVEN a Abacus mapping with or without some trustzones
        mapping = AbacusMapping(trustzones=trustzone_mappings, components=[])
    ...
    @patch('slp_abacus.slp_abacus.parse.diagram_mapper._find_mapping', wraps=_find_mapping)
    ...
    def test_mapping_components(self, find_mapping_wrapper, component: DiagramComponent, mappings: List,
        ...
        # AND a AbacusMapping with some component mappings matching by exact name/type or regex
        abacus_mapping = AbacusMapping(trustzones=[], components=mappings)

        # WHEN DiagramMapper::map is invoked
        DiagramMapper(diagram=diagram, mapping=abacus_mapping).map()
    ...
    def test_mapping_trustzones(self, find_mapping_wrapper, component: DiagramComponent, mappings: List,
        ...
        # AND a AbacusMapping with some trustzone mappings matching by exact name/type or regex
        mapping = AbacusMapping(trustzones=mappings, components=[])
    ...
    def test_trustzone_over_component(self):
        ...
        mapping = AbacusMapping(components=[mapping_by_name], trustzones=[mapping_by_name])
    ...
    def test_non_mapped_components(self):
        ...
        mapping = AbacusMapping(components=[non_matching_mapping], trustzones=[non_matching_mapping])
    ...
    def test_no_mappings(self):
        ...
        # AND a Abacus mapping with no mappings
        mapping = AbacusMapping(components=[], trustzones=[])
    ...
    def test_no_components(self):
        ...
        # AND a AbacusMapping with mappings
        # AND a default trustzone
        default_trustzone_mapping = _tz_mapping(default=True)
        mapping = AbacusMapping(components=[{'label': 'label', 'type': 'type'}], trustzones=[default_trustzone_mapping])
