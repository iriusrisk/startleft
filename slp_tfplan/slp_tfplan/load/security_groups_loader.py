...
def _get_sg_rules_by_sg_id(sg_id: str, rule_type: Literal['ingress', 'egress'], sg_rules: List[Dict[str, str]]
                           ) -> List[Dict[str, str]]:
    return list(filter(
        lambda sg_rule:
        sg_rule['security_group_id'] == sg_id and sg_rule['type'] == rule_type,
        sg_rules))


def _source_security_group_ids_of_type_from_rule(security_group: Dict, rule_type: Literal['ingress', 'egress'],
...
def _cidr_of_type_from_rule(security_group: Dict, rule_type: Literal['ingress', 'egress'],
                            sg_rules: List[Dict[str, str]]):
    sg_rules_by_sg_id = _get_sg_rules_by_sg_id(security_group['resource_id'], rule_type, sg_rules)
...
def _get_sgs_of_type(security_group: Dict, sg_type: Literal['ingress', 'egress'],
                     sg_rules: List[Dict[str, str]]) -> List[str]:
    return security_groups_ids_from_type_property(security_group, sg_type) or \
...
def _get_cidr_of_type(security_group: Dict, cidr_type: Literal['ingress', 'egress'],
                      sg_rules: List[Dict[str, str]]) -> List[SecurityGroupCIDR]:
    sg_cidr = cidr_from_type_property(security_group, cidr_type) or \
    ...
    return list(map(lambda cidr:
                    SecurityGroupCIDRLoader(cidr).load(),
                    sg_cidr))


class SecurityGroupCIDRLoader:

    def __init__(self, security_group_cidr: dict):
...
class SecurityGroupsLoader:
    ...
    def __build_security_group(self, resource: {}) -> SecurityGroup:
        return SecurityGroup(
            ...
            ingress_sgs=_get_sgs_of_type(resource, 'ingress', self.sg_rules),
            egress_sgs=_get_sgs_of_type(resource, 'egress', self.sg_rules),
            ingress_cidr=_get_cidr_of_type(resource, 'ingress', self.sg_rules),
            egress_cidr=_get_cidr_of_type(resource, 'egress', self.sg_rules),
        )
